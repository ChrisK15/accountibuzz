...
2. **Realtime subscription life-cycle hardening** — events between tab unmount/remount can drop. Currently mitigated by aggressive `invalidateQueries` on mutations. Phase 3.1 may reintroduce a presence-aware re-fetch on focus.
   - **2026-05-09 confirmed via Phase 4 UAT (CK-05 setup)**: when Chris K2 submitted on Device B, admin "funny guy" on Device A's group-detail "Pending Review" queue did NOT auto-update with the new pending submission — admin had to back out and re-enter the screen to see it. Phase 4's CK-05 was unblocked by approving directly via Supabase MCP rather than through the admin UI flow. The relevant hook is the P3 admin pending-review Realtime subscription in `src/features/submissions/` (likely `useReviewQueueRealtime` or whatever the P3-shipped variant is named). Concrete repro for Phase 3.1: Device A on group-detail as admin; Device B submits a fresh photo/video via capture flow; observe Device A's Pending Review queue does NOT auto-prepend the new pending row within ~2s.
