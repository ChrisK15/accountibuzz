...
| pgTAP (`supabase test db`) | **107 / 107 assertions** in 17 / 17 files | Runs against migration `0006_phase3_capture_review` + `0007` realtime publication on the live remote. Includes 4 new P3 test files: `submit_today.sql`, `review_submission.sql`, `get_pending_review_count.sql`, `get_pending_review_queue.sql`, plus the `submissions_admin_immutable.sql` 0003-trigger backfill. |
